...
var utils = {
  ...
  sha256: async (message) => {
    if (crypto.web) {
      ...
      return Uint8Array.from(createHash2("sha256").update(message).digest());
    } else {
...
function relayPool() {
  ...
  const sub = ({ cb, filter, beforeSend }, id = Math.random().toString().slice(2)) => {
    ...
    const sub2 = ({
      ...
      Object.entries(subControllers).map(([relayURL, sub3]) => [
        relayURL,
        sub3.sub({ cb: cb2, filter: filter2, beforeSend: beforeSend2 }, id)
      ]);
